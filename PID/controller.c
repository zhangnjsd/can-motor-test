...
float PID_Calculate_yaw(PID_t *pid, float measure, float ref)
{
    ...
    if (abs(pid->Err) > pid->DeadBand)
    {
        if (pid->FuzzyRule == NULL)
        {
            pid->Pout = pid->Kp * pid->Err;
            pid->ITerm = pid->Ki * pid->Err * pid->dt;
            pid->Dout = pid->Kd * INS.Gyro[2];
        }
        else
        {
            pid->Pout = (pid->Kp + pid->FuzzyRule->KpFuzzy) * pid->Err;
            pid->ITerm = (pid->Ki + pid->FuzzyRule->KiFuzzy) * pid->Err * pid->dt;
            pid->Dout = (pid->Kd + pid->FuzzyRule->KdFuzzy) * INS.Gyro[2];
...
float PID_Calculate_pitch(PID_t *pid, float measure, float ref)
{
    ...
    if (abs(pid->Err) > pid->DeadBand)
    {
        if (pid->FuzzyRule == NULL)
        {
            ...
            pid->ITerm = pid->Ki * pid->Err * pid->dt;
			//鏀规��  寮€涔呬簡yaw杞存檭鍔ㄨ繃澶?
            pid->Dout = pid->Kd * INS.Gyro[0];
        }
        else
        {
            pid->Pout = (pid->Kp + pid->FuzzyRule->KpFuzzy) * pid->Err;
            pid->ITerm = (pid->Ki + pid->FuzzyRule->KiFuzzy) * pid->Err * pid->dt;
            pid->Dout = (pid->Kd + pid->FuzzyRule->KdFuzzy) * INS.Gyro[0];
...
{
    if (pid->FuzzyRule == NULL)
        pid->ITerm = pid->Ki * ((pid->Err + pid->Last_Err) / 2) * pid->dt;  //梯形积分ki部分 
    else
...
static void f_Changing_Integration_Rate(PID_t *pid)
{
    ...
    {
        // 积分呈累积趋势
        if (abs(pid->Err) <= pid->CoefB)			 	//|Err| <= B
            return; // 完整积分		
        if (abs(pid->Err) <= (pid->CoefA + pid->CoefB)) 	// B <= |Err| <= A+B
		  //使用线性函数过度
...
/**
 * @brief     积分抗饱和  
			思路一：PID输出达到输出限幅时停止积分过程
			思路二：设定一个合适的积分阈值，来通过这个阈值对积分进行限幅
 * @param[in]      PID结构体
 * @retval         返回空
 */
static void f_Integral_Limit(PID_t *pid)
{
    ...
    if (abs(temp_Output) > pid->MaxOut)     		//思路一
    {
        if (pid->Err * pid->Iout > 0)
        {
    ...
    if (temp_Iout > pid->IntegralLimit)			//思路二 设定积分阈值并进行限幅
    {
        pid->ITerm = 0;
...
static void f_Derivative_On_Measurement(PID_t *pid)
{
    if (pid->FuzzyRule == NULL)
    {
        if (pid->OLS_Order > 2)
            ...
            pid->Dout = pid->Kd * (pid->Last_Measure - pid->Measure) / pid->dt; //扔掉目标信号的微分，只保留输入（测量值）的微分 
    }
    else
    {
        if (pid->OLS_Order > 2)
...
static void f_PID_ErrorHandle(PID_t *pid)
{
    ...
    if ((fabsf(pid->Ref - pid->Measure) / fabsf(pid->Ref)) > 0.95f) // 0.95  选值应灵活
    {
...
void Feedforward_Init(
    Feedforward_t *ffc,  	//结构体
    float max_out,		//最大输出	
    float *c,			//数组指针  各部分权重
    float lpf_rc,			//低通滤波参数
    uint16_t ref_dot_ols_order, //10  20
    uint16_t ref_ddot_ols_order)//10 20 上下两者一致
{
    ffc->MaxOut = max_out;
...
/*实现前馈控制器*/
float FeedforwardController(FFC_typedef *vFFC)
{
  float result;
